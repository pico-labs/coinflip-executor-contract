import { isReady, shutdown, Mina, PrivateKey, AccountUpdate, MerkleMap, Field, Poseidon,
// Circuit,
// Signature,
 } from 'snarkyjs';
...
describe('executor', () => {
    ...
    describe('deposit', () => {
        it('sets initial balance', async () => {
            ...
            await tx2.send();
            // test
            expect(Mina.getBalance(executorAddress).toString()).toBe(executorBalance.add(1000).toString());
            expect(Mina.getBalance(player1PrivateKey.toPublicKey()).toString()).toBe(playerBalance.sub(1000).toString());
        ...
        it('increases existing balance', async () => {
            ...
            const tx3 = await Mina.transaction(deployerPrivateKey, () => {
                executor.deposit(player1PrivateKey.toPublicKey(), Field(50), Field(1000), witness);
            });
            await tx3.prove();
            tx3.sign([player1PrivateKey]);
            await tx3.send();
            // test
            expect(Mina.getBalance(executorAddress).toString()).toBe(executorBalance.add(1050).toString());
            expect(Mina.getBalance(player1PrivateKey.toPublicKey()).toString()).toBe(playerBalance.sub(1050).toString());
            const appState = Mina.getAccount(executorAddress).appState;
            expect(appState).not.toBeNull();
            if (appState) {
    ...
    describe('withdraw', () => {
        it('deposits and withdraws', async () => {
            ...
            const tx3 = await Mina.transaction(player1PrivateKey, () => {
                executor.withdraw(player1PrivateKey.toPublicKey(), Field(1000), witness);
            });
            await tx3.prove();
            await tx3.send();
            //test
            expect(Mina.getBalance(executorAddress).toString()).toBe(executorBalance.toString());
            expect(Mina.getBalance(player1PrivateKey.toPublicKey()).toString()).toBe(playerBalance.toString());
            const appState = Mina.getAccount(executorAddress).appState;
            expect(appState).not.toBeNull();
            if (appState) {
    ...
    //     ]);
    //     Circuit.runAndCheck(() => {
    //       const incomingDelta = executor.flipCoin(
    //         player1PrivateKey.toPublicKey(),
    //         merkleMap.get(key),
    //         witness,
